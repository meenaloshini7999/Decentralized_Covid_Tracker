...
contract Main {
  ...
  mapping(string=>string) public familyHealthRecord;
  ...
  function createReporter(string memory _id,string memory _name,uint _pincode,string memory _flatid){
    require(lAdmins[_pincode].ladminadd==msg.sender,"Access Denied");
    reporters[_id]=Reporter(_id,_name,_pincode,_flatid);
  }

  function createDoctor(uint _pincode,address _address,string memory _id,string memory _workarea,string memory _flatid){
    require(lAdmins[_pincode].ladminadd==msg.sender,"Access Denied");
    doctors[_id]=Doctor(_address,_id,_workarea,_flatid);
  }

  function createFamily(uint _pincode,string memory _name,string memory _id,string memory _aadhaar,string memory _home,string memory _curr,bool _flag) {
    require(lAdmins[_pincode].ladminadd==msg.sender,"Access Denied");
    families[_id]=Family(_name,_id,_aadhaar,_home,_curr,_flag);
  }

  function createFlat(uint _pincode,string memory _id,uint _total,uint _active,uint _recovered,string memory _coords) {
    require(lAdmins[_pincode].ladminadd==msg.sender,"Access Denied");
    flats[_id]=Flat(_id,_total,_active,_recovered,_total-_active-_recovered,_coords);
